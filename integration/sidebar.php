<div class="sidebar-mask"></div>
<aside class="sidebar">
        <!-- <nav class="nav-links">
          <div class="nav-item"><a
              href="https://uchat-com-au.atlassian.net/wiki/spaces/UKB/pages/668598387/UChat+Introduction"
              target="_blank" rel="noopener noreferrer" class="nav-link external">
              New Help Center
              <span><svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false" x="0px" y="0px"
                  viewBox="0 0 100 100" width="15" height="15" class="icon outbound">
                  <path fill="currentColor"
                    d="M18.8,85.1h56l0,0c2.2,0,4-1.8,4-4v-32h-8v28h-48v-48h28v-8h-32l0,0c-2.2,0-4,1.8-4,4v56C14.8,83.3,16.6,85.1,18.8,85.1z">
                  </path>
                  <polygon fill="currentColor"
                    points="45.7,48.7 51.3,54.3 77.2,28.5 77.2,37.2 85.2,37.2 85.2,14.9 62.8,14.9 62.8,22.9 71.5,22.9">
                  </polygon>
                </svg> <span class="sr-only">(opens new window)</span></span></a></div>
          <div class="nav-item"><a href="../guide/index.php" class="nav-link">
              Guide
            </a></div>
          <div class="nav-item">
            <div class="dropdown-wrapper"><button type="button" aria-label="Channels" class="dropdown-title"><span
                  class="title">Channels</span> <span class="arrow right"></span></button>
              <ul class="nav-dropdown" style="display:none;">
                <li class="dropdown-item"> <a href="../guide/setup-create.php#omni-bot" class="nav-link">
                    Omnichannel
                  </a></li>
                <li class="dropdown-item"><a href="../guide/setup-create.php#facebook-bot" class="nav-link">
                    Facebook
                  </a></li>
                <li class="dropdown-item"><a href="../guide/setup-create.php#instagram-bot" class="nav-link">
                    Instagram
                  </a></li>
                <li class="dropdown-item"><a href="../guide/setup-create.php#telegram-bot" class="nav-link">
                    Telegram
                  </a></li>
                <li class="dropdown-item"><a href="../guide/setup-create.php#slack-bot" class="nav-link">
                    Slack
                  </a></li>
                <li class="dropdown-item"><a href="../guide/setup-create.php#wechat-bot" class="nav-link">
                    WeChat
                  </a></li>
                <li class="dropdown-item"><a href="../guide/setup-create.php#whatsapp-360dialog-sandbox"
                    class="nav-link">
                    WhatsApp Sandbox
                  </a></li>
                <li class="dropdown-item"><a href="../guide/whatsapp-api.php" class="nav-link">
                    WhatsApp
                  </a></li>
                <li class="dropdown-item"><a href="../guide/cloudapi.php" class="nav-link">
                    WhatsApp Cloud API
                  </a></li>
                <li class="dropdown-item"><a href="../guide/setup-create.php#whatsapp-sms-voice-bot"
                    class="nav-link">
                    SMS
                  </a></li>
                <li class="dropdown-item"><a href="../guide/setup-create.php#whatsapp-sms-voice-bot"
                    class="nav-link">
                    Voice
                  </a></li>
                <li class="dropdown-item"><a href="../guide/setup-create.php#google-my-business"
                    class="nav-link">
                    Google
                  </a></li>
                <li class="dropdown-item"><a href="https://help.uchat.com.au/line-chatbot" target="_blank"
                    rel="noopener noreferrer" class="nav-link external">
                    Line
                    <span><svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false" x="0px" y="0px"
                        viewBox="0 0 100 100" width="15" height="15" class="icon outbound">
                        <path fill="currentColor"
                          d="M18.8,85.1h56l0,0c2.2,0,4-1.8,4-4v-32h-8v28h-48v-48h28v-8h-32l0,0c-2.2,0-4,1.8-4,4v56C14.8,83.3,16.6,85.1,18.8,85.1z">
                        </path>
                        <polygon fill="currentColor"
                          points="45.7,48.7 51.3,54.3 77.2,28.5 77.2,37.2 85.2,37.2 85.2,14.9 62.8,14.9 62.8,22.9 71.5,22.9">
                        </polygon>
                      </svg> <span class="sr-only">(opens new window)</span></span></a></li>
                <li class="dropdown-item"><a href="https://help.uchat.com.au/viber-chatbot" target="_blank"
                    rel="noopener noreferrer" class="nav-link external">
                    Viber
                    <span><svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false" x="0px" y="0px"
                        viewBox="0 0 100 100" width="15" height="15" class="icon outbound">
                        <path fill="currentColor"
                          d="M18.8,85.1h56l0,0c2.2,0,4-1.8,4-4v-32h-8v28h-48v-48h28v-8h-32l0,0c-2.2,0-4,1.8-4,4v56C14.8,83.3,16.6,85.1,18.8,85.1z">
                        </path>
                        <polygon fill="currentColor"
                          points="45.7,48.7 51.3,54.3 77.2,28.5 77.2,37.2 85.2,37.2 85.2,14.9 62.8,14.9 62.8,22.9 71.5,22.9">
                        </polygon>
                      </svg> <span class="sr-only">(opens new window)</span></span></a></li>
                <li class="dropdown-item"> <a href="../guide/setup-create.php#vk-bot" class="nav-link">
                    Vk
                  </a></li>
              </ul>
            </div>
          </div>
          <div class="nav-item"><a href="../flow-builder/index.php" class="nav-link">
              Flow Builder
            </a></div>
          <div class="nav-item">
            <div class="dropdown-wrapper"><button type="button" aria-label="Integration" class="dropdown-title"><span
                  class="title">Integration</span> <span class="arrow right"></span></button>
              <ul class="nav-dropdown" style="display:none;">
                <li class="dropdown-item"><a href="index.php" aria-current="page"
                    class="nav-link router-link-exact-active router-link-active">
                    Overview
                  </a></li>
                <li class="dropdown-item"><a href="stripe.php" class="nav-link">
                    Stripe
                  </a></li>
                <li class="dropdown-item"><a href="mark/paypal.php" class="nav-link">
                    PayPal
                  </a></li>
                <li class="dropdown-item"><a href="mark/cloudinary.php" class="nav-link">
                    Cloudinary
                  </a></li>
                <li class="dropdown-item"><a href="shopify.php" class="nav-link">
                    Shopify
                  </a></li>
                <li class="dropdown-item"><a href="fb-manager.php" class="nav-link">
                    Facebook Business
                  </a></li>
                <li class="dropdown-item"><a href="mark/calendly.php" class="nav-link">
                    Calendly
                  </a></li>
                <li class="dropdown-item"><a href="slack.php" class="nav-link">
                    Slack
                  </a></li>
                <li class="dropdown-item"><a href="slack-agent.php" class="nav-link">
                    Slack Agent
                  </a></li>
                <li class="dropdown-item"><a href="dialogflow.php" class="nav-link">
                    Dialogflow
                  </a></li>
                <li class="dropdown-item"><a href="google-spreadsheet.php" class="nav-link">
                    Google Spreadsheet
                  </a></li>
                <li class="dropdown-item"><a href="data-store.php" class="nav-link">
                    Data Store
                  </a></li>
                <li class="dropdown-item"><a href="mark/twilio.php" class="nav-link">
                    Twilio
                  </a></li>
                <li class="dropdown-item"><a href="mark/signalwire.php" class="nav-link">
                    SignalWire
                  </a></li>
                <li class="dropdown-item"><a href="email.php" class="nav-link">
                    SMTP / Gmail / SendGrid
                  </a></li>
                <li class="dropdown-item"><a href="mailchimp.php" class="nav-link">
                    MailChimp
                  </a></li>
                <li class="dropdown-item"><a href="mark/zapier.php" class="nav-link">
                    Zapier
                  </a></li>
                <li class="dropdown-item"><a href="mark/integromat.php" class="nav-link">
                    Integromat
                  </a></li>
                <li class="dropdown-item"><a href="mark/pabbly.php" class="nav-link">
                    Pabbly
                  </a></li>
                <li class="dropdown-item"><a href="mark/integrately.php" class="nav-link">
                    Integrately
                  </a></li>
                <li class="dropdown-item"><a href="mark/videosdk.php" class="nav-link">
                    VideoSdk
                  </a></li>
                <li class="dropdown-item"><a href="intercom.php" class="nav-link">
                    Intercom
                  </a></li>
                <li class="dropdown-item"><a href="freshchat.php" class="nav-link">
                    Freshchat
                  </a></li>
              </ul>
            </div>
          </div>
          <div class="nav-item"><a href="../ecommerce/index.php" class="nav-link">
              Ecommerce
            </a></div>
          <div class="nav-item"><a href="../mobile/mobile.php" class="nav-link">
              Mobile App
            </a></div>
          <div class="nav-item">
            <div class="dropdown-wrapper"><button type="button" aria-label="For Developers" class="dropdown-title"><span
                  class="title">For Developers</span> <span class="arrow right"></span></button>
              <ul class="nav-dropdown" style="display:none;">
                <li class="dropdown-item"><a href="../for-developers/index.php" class="nav-link">
                    Dynamic Content
                  </a></li>
                <li class="dropdown-item"><a href="../for-developers/API/index.php" class="nav-link">
                    API
                  </a></li>
                <li class="dropdown-item"><a href="../for-developers/my-apps/index.php" class="nav-link">
                    Mini-App
                  </a></li>
              </ul>
            </div>
          </div>
          <div class="nav-item"><a href="../partner/about.php" class="nav-link">
              Partner
            </a></div>
          <div class="nav-item"><a href="../faq/index.php" class="nav-link">
              FAQ
            </a></div>
          <div class="nav-item"><a href="https://www.uchat.com.au/" target="_blank" rel="noopener noreferrer"
              class="nav-link external">
              UChat Home
              <span><svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false" x="0px" y="0px"
                  viewBox="0 0 100 100" width="15" height="15" class="icon outbound">
                  <path fill="currentColor"
                    d="M18.8,85.1h56l0,0c2.2,0,4-1.8,4-4v-32h-8v28h-48v-48h28v-8h-32l0,0c-2.2,0-4,1.8-4,4v56C14.8,83.3,16.6,85.1,18.8,85.1z">
                  </path>
                  <polygon fill="currentColor"
                    points="45.7,48.7 51.3,54.3 77.2,28.5 77.2,37.2 85.2,37.2 85.2,14.9 62.8,14.9 62.8,22.9 71.5,22.9">
                  </polygon>
                </svg> <span class="sr-only">(opens new window)</span></span></a></div>
        </nav> -->
        <ul class="sidebar-links">
            <li>
                <section class="sidebar-group depth-0">
                    <p class="sidebar-heading open">
                        <span>Integration</span>
                    </p>
                    <ul class="sidebar-links sidebar-group-items">
                        <li><a href="index.php" aria-current="page" class="active sidebar-link">Integrations</a></li>
                        <li>
                            <a href="../integration/stripe.php" class="sidebar-link">Stripe</a>
                            <ul class="sidebar-sub-headers">
                                <li class="sidebar-sub-header">
                                    <a href="stripe.php#setup-stripe" class="sidebar-link">
                                        Setup Stripe
                                    </a>
                                </li>
                                <li class="sidebar-sub-header">
                                    <a href="stripe.php#use-stripe" class="sidebar-link">
                                        Use Stripe
                                    </a>
                                </li>
                            </ul>
                        </li>
                        <li>
                            <a href="mark/paypal.php" class="sidebar-link">PayPal</a>
                            <ul class="sidebar-sub-headers">
                                <li class="sidebar-sub-header">
                                    <a href="paypal.php#login-to-your-paypal-developer-account" class="sidebar-link">Login to your PayPal developer account</a>
                                </li>
                                <li class="sidebar-sub-header">
                                    <a href="paypal.php#insert-your-paypal-details" class="sidebar-link">Insert your PayPal details</a>
                                </li>
                                <li class="sidebar-sub-header">
                                    <a href="paypal.php#how-to-use-paypal-in-your-bot" class="sidebar-link">How to use PayPal in your bot</a>
                                </li>
                            </ul>
                        </li>
                        <li>
                            <a href="mark/cloudinary.php" class="sidebar-link">Cloudinary</a>
                            <ul class="sidebar-sub-headers">
                                <li class="sidebar-sub-header">
                                    <a href="cloudinary.php#login-to-cloudinary" class="sidebar-link">Login to Cloudinary</a>
                                </li>
                                <li class="sidebar-sub-header">
                                    <a href="cloudinary.php#creating-your-api-key" class="sidebar-link">Creating your API key</a>
                                </li>
                                <li class="sidebar-sub-header">
                                    <a href="cloudinary.php#using-cloudinary-in-your-flows" class="sidebar-link">Using Cloudinary in your flows</a>
                                </li>
                            </ul>
                        </li>
                        <li>
                            <a href="shopify.php" class="sidebar-link">Shopify</a>
                            <ul class="sidebar-sub-headers">
                                <li class="sidebar-sub-header">
                                    <a href="shopify.php#setup-shopify" class="sidebar-link">Setup Shopify</a>
                                </li>
                                <li class="sidebar-sub-header">
                                    <a href="shopify.php#use-shopify-in-action-step" class="sidebar-link">Use Shopify in Action Step</a>
                                </li>
                            </ul>
                        </li>
                        <li>
                            <a href="fb-manager.php" class="sidebar-link">Facebook Business Management</a>
                            <ul class="sidebar-sub-headers">
                                <li class="sidebar-sub-header">
                                    <a href="fb-manager.php#connect-to-your-facebook-business-account" class="sidebar-link">
                                        Connect to your Facebook business account
                                    </a>
                                </li>
                                <li class="sidebar-sub-header">
                                    <a href="fb-manager.php#disconnect-your-facebook-business-account" class="sidebar-link">
                                        Disconnect your Facebook business account
                                    </a>
                                </li>
                                <li class="sidebar-sub-header">
                                    <a href="fb-manager.php#sync-the-product-catalog-from-facebook-to-your-ecommerce" class="sidebar-link">
                                        Sync the product catalog from Facebook to your ecommerce
                                    </a>
                                </li>
                                <li class="sidebar-sub-header">
                                    <a href="fb-manager.php#change-the-currency" class="sidebar-link">
                                        Change the currency
                                    </a>
                                </li>
                                <li class="sidebar-sub-header">
                                    <a href="fb-manager.php#benefits-of-syncing-facebook-catalog-with-uchat" class="sidebar-link">
                                        Benefits of syncing Facebook catalog with Smart ChatBot
                                    </a>
                                </li>
                            </ul>
                        </li>
                        <li>
                            <a href="mark/calendly.php" class="sidebar-link">Calendly</a>
                            <ul class="sidebar-sub-headers">
                                <li class="sidebar-sub-header">
                                    <a href="calendly.php#connect-to-calendly" class="sidebar-link">
                                        Connect to Calendly
                                    </a>
                                </li>
                                <li class="sidebar-sub-header">
                                    <a href="calendly.php#how-to-use-calendly" class="sidebar-link">
                                        How to use Calendly
                                    </a>
                                </li>
                            </ul>
                        </li>
                        <li>
                            <a href="slack.php" class="sidebar-link">Slack</a>
                            <ul class="sidebar-sub-headers">
                                <li class="sidebar-sub-header">
                                    <a href="slack.php#setup-slack" class="sidebar-link">Setup Slack</a>
                                </li>
                                <li class="sidebar-sub-header">
                                    <a href="slack.php#send-slack-notification" class="sidebar-link">Send Slack Notification</a>
                                </li>
                            </ul>
                        </li>
                        <li>
                            <a href="slack-agent.php" class="sidebar-link">Slack Agent</a>
                            <ul class="sidebar-sub-headers">
                                <li class="sidebar-sub-header">
                                    <a href="slack-agent.php#connect-slack-to-your-workspace" class="sidebar-link">
                                        Connect Slack to your workspace
                                    </a>
                                </li>
                                <li class="sidebar-sub-header">
                                    <a href="slack-agent.php#how-to-use-activate-slack-as-a-live-chat" class="sidebar-link">
                                        How to use Activate Slack as a live chat
                                    </a>
                                </li>
                                <li class="sidebar-sub-header">
                                    <a href="slack-agent.php#how-to-use-slack-as-a-live-chat" class="sidebar-link">
                                        How to use Slack as a live chat
                                    </a>
                                </li>
                                <li class="sidebar-sub-header">
                                    <a href="slack-agent.php#using-slash-commands" class="sidebar-link">
                                        Using Slash Commands
                                    </a>
                                </li>
                                <li class="sidebar-sub-header">
                                    <a href="slack-agent.php#add-uchat-agent-bot-to-your-slack" class="sidebar-link">
                                        Add Smart ChatBot Agent bot to your Slack
                                    </a>
                                </li>
                                <li class="sidebar-sub-header">
                                    <a href="slack-agent.php#reach-out-to-support" class="sidebar-link">
                                        Reach out to support
                                    </a>
                                </li>
                                <li class="sidebar-sub-header">
                                    <a href="slack-agent.php#privacy-policy" class="sidebar-link">
                                        Privacy Policy
                                    </a>
                                </li>
                            </ul>
                        </li>
                        <li>
                            <a href="dialogflow.php" class="sidebar-link">Dialogflow</a>
                            <ul class="sidebar-sub-headers">
                                <li class="sidebar-sub-header">
                                    <a href="dialogflow.php#setup-dialogflow" class="sidebar-link">
                                        Setup Dialogflow
                                    </a>
                                </li>
                                <li class="sidebar-sub-header">
                                    <a href="dialogflow.php#use-dialogflow" class="sidebar-link">
                                        Use Dialogflow
                                    </a>
                                </li>
                                <li class="sidebar-sub-header">
                                    <a href="dialogflow.php#improve-dialogflow" class="sidebar-link">
                                        Improve Dialogflow
                                    </a>
                                </li>
                                <li class="sidebar-sub-header">
                                    <a href="dialogflow.php#dynamic-content-response" class="sidebar-link">
                                        Dynamic Content Response
                                    </a>
                                </li>
                            </ul>
                        </li>
                        <li>
                            <a href="google-spreadsheet.php" class="sidebar-link">Google Spreadsheet</a>
                            <ul class="sidebar-sub-headers">
                                <li class="sidebar-sub-header">
                                    <a href="google-spreadsheet.php#setup-google-spreadsheet" class="sidebar-link">
                                        Setup Google Spreadsheet
                                    </a>
                                </li>
                                <li class="sidebar-sub-header">
                                    <a href="google-spreadsheet.php#manage-spreadsheet-in-action-step" class="sidebar-link">
                                        Manage Spreadsheet in Action Step
                                    </a>
                                </li>
                            </ul>
                        </li>
                        <li>
                            <a href="data-store.php" class="sidebar-link">Data Store</a>
                            <ul class="sidebar-sub-headers">
                                <li class="sidebar-sub-header">
                                    <a href="data-store.php#setup-data-store" class="sidebar-link">
                                        Setup Data Store
                                    </a>
                                </li>
                                <li class="sidebar-sub-header">
                                    <a href="data-store.php#manage-records-manually" class="sidebar-link">
                                        Manage Records Manually
                                    </a>
                                </li>
                                <li class="sidebar-sub-header">
                                    <a href="data-store.php#manage-records-in-action-step" class="sidebar-link">
                                        Manage Records in Action Step
                                    </a>
                                </li>
                            </ul>
                        </li>
                        <li>
                            <a href="mark/twilio.php" class="sidebar-link">Twilio</a>
                            <ul class="sidebar-sub-headers">
                                <li class="sidebar-sub-header">
                                    <a href="twilio.php#login-to-your-twilio-account" class="sidebar-link">
                                        Login to your Twilio account
                                    </a>
                                </li>
                                <li class="sidebar-sub-header">
                                    <a href="twilio.php#how-to-use-twilio" class="sidebar-link">
                                        How to use Twilio
                                    </a>
                                </li>
                            </ul>
                        </li>
                        <li>
                            <a href="mark/signalwire.php" class="sidebar-link">Signalwire</a>
                            <ul class="sidebar-sub-headers">
                                <li class="sidebar-sub-header">
                                    <a href="signalwire.php#login-to-signalwire" class="sidebar-link">
                                        Login to Signalwire
                                    </a>
                                </li>
                                <li class="sidebar-sub-header">
                                    <a href="signalwire.php#creating-or-fetching-your-token" class="sidebar-link">
                                        Creating or fetching your token
                                    </a>
                                </li>
                                <li class="sidebar-sub-header">
                                    <a href="signalwire.php#how-to-use-signalwire" class="sidebar-link">
                                        How to use SignalWire
                                    </a>
                                </li>
                            </ul>
                        </li>
                        <li>
                            <a href="email.php" class="sidebar-link">Email (SMTP / Gmail / Sendgrid)</a>
                            <ul class="sidebar-sub-headers">
                                <li class="sidebar-sub-header">
                                    <a href="email.php#smtp-profile-setup" class="sidebar-link">
                                        SMTP Profile Setup
                                    </a>
                                </li>
                                <li class="sidebar-sub-header">
                                    <a href="email.php#gmail-setup" class="sidebar-link">
                                        Gmail Setup
                                    </a>
                                </li>
                                <li class="sidebar-sub-header">
                                    <a href="email.php#sendgrid-setup" class="sidebar-link">
                                        SendGrid Setup
                                    </a>
                                </li>
                                <li class="sidebar-sub-header">
                                    <a href="email.php#send-email-notification" class="sidebar-link">
                                        Send Email Notification
                                    </a>
                                </li>
                                <li class="sidebar-sub-header">
                                    <a href="email.php#send-email-step" class="sidebar-link">
                                        Send Email Step
                                    </a>
                                </li>
                            </ul>
                        </li>
                        <li>
                            <a href="mailchimp.php" class="sidebar-link">MailChimp</a>
                            <ul class="sidebar-sub-headers">
                                <li class="sidebar-sub-header">
                                    <a href="mailchimp.php#setup" class="sidebar-link">
                                        Setup
                                    </a>
                                </li>
                                <li class="sidebar-sub-header">
                                    <a href="mailchimp.php#use-mailchimp-in-action-step" class="sidebar-link">
                                        Use MailChimp in Action Step
                                    </a>
                                </li>
                            </ul>
                        </li>
                        <li>
                            <a href="mark/zapier.php" class="sidebar-link">Zapier</a>
                            <ul class="sidebar-sub-headers">
                                <li class="sidebar-sub-header">
                                    <a href="zapier.php#how-to-add-uchat-to-your-zapier-account" class="sidebar-link">
                                        How to add Smart ChatBot to your Zapier account
                                    </a>
                                </li>
                                <li class="sidebar-sub-header">
                                    <a href="zapier.php#how-to-use-zapier" class="sidebar-link">
                                        How to use Zapier
                                    </a>
                                </li>
                            </ul>
                        </li>
                        <li>
                            <a href="mark/integromat.php" class="sidebar-link">Integromat</a>
                            <ul class="sidebar-sub-headers">
                                <li class="sidebar-sub-header">
                                    <a href="integromat.php#how-to-add-uchat-to-your-integromat-account" class="sidebar-link">
                                        How to add Smart ChatBot to your Integromat account
                                    </a>
                                </li>
                                <li class="sidebar-sub-header">
                                    <a href="integromat.php#how-to-use-integromat" class="sidebar-link">
                                        How to use Integromat
                                    </a>
                                </li>
                            </ul>
                        </li>
                        <li>
                            <a href="mark/pabbly.php" class="sidebar-link">Pabbly</a>
                            <ul class="sidebar-sub-headers">
                                <li class="sidebar-sub-header">
                                    <a href="pabbly.php#how-to-add-uchat-to-your-pabbly-account"
                        class="sidebar-link">How to add Smart ChatBot to your Pabbly account</a></li>
                  </ul>
                        </li>
                        <li>
                            <a href="mark/integrately.php" class="sidebar-link">Integrately</a>
                            <ul class="sidebar-sub-headers">
                                <li class="sidebar-sub-header">
                                    <a href="integrately.php#how-to-add-uchat-to-your-integrately-account" class="sidebar-link">
                                        How to add Smart ChatBot to your Integrately account
                                    </a>
                                </li>
                                <li class="sidebar-sub-header">
                                    <a href="integrately.php#how-to-use-the-uchat-app-in-your-integrately-account" class="sidebar-link">
                                        How to use the Smart ChatBot app in your Integrately account
                                    </a>
                                </li>
                            </ul>
                        </li>
                        <li>
                            <a href="mark/videosdk.php" class="sidebar-link">VideoSdk</a>
                            <ul class="sidebar-sub-headers">
                                <li class="sidebar-sub-header">
                                    <a href="videosdk.php#how-to-fetch-your-credentials" class="sidebar-link">
                                        How to fetch your credentials
                                    </a>
                                </li>
                                <li class="sidebar-sub-header">
                                    <a href="videosdk.php#how-to-use-videosdk-in-your-chatbot" class="sidebar-link">
                                        How to use VideoSdk in your chatbot
                                    </a>
                                </li>
                            </ul>
                        </li>
                        <li>
                            <a href="intercom.php" class="sidebar-link">Intercom</a>
                            <ul class="sidebar-sub-headers">
                                <li class="sidebar-sub-header">
                                    <a href="intercom.php#key-benefits" class="sidebar-link">
                                        Key Benefits
                                    </a>
                                </li>
                                <li class="sidebar-sub-header">
                                    <a href="intercom.php#omnichannel-flow-builder" class="sidebar-link">
                                        Omnichannel flow builder
                                    </a>
                                </li>
                                <li class="sidebar-sub-header">
                                    <a href="intercom.php#connect-to-intercom" class="sidebar-link">
                                        Connect to Intercom
                                    </a>
                                </li>
                                <li class="sidebar-sub-header">
                                    <a href="intercom.php#use-intercom" class="sidebar-link">
                                        Use Intercom
                                    </a>
                                </li>
                            </ul>
                        </li>
                        <li>
                            <a href="freshchat.php" class="sidebar-link">Freshchat</a>
                            <ul class="sidebar-sub-headers">
                                <li class="sidebar-sub-header">
                                    <a href="freshchat.php#how-to-set-up-your-freshchat-integration" class="sidebar-link">
                                        How to set up your Freshchat integration
                                    </a>
                                </li>
                                <li class="sidebar-sub-header">
                                    <a href="freshchat.php#how-to-trigger-live-chat-to-freshchat" class="sidebar-link">
                                        How to trigger live chat to Freshchat
                                    </a>
                                </li>
                            </ul>
                        </li>
                    </ul>
                </section>
            </li>
        </ul>
    </aside>